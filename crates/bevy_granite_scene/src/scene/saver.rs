...
impl<'a, W: std::fmt::Write> SceneSaver<'a, W> {
    ...
    fn serialize_entities(&mut self) -> Result<()> {
        writeln!(&mut self.file, "{}", Chapters::Entities)?;
        let register = self.register.read();
        let mut entitiy_serializer: crate::reflect_serde::EntitySerializer<'_, W> =
            crate::reflect_serde::EntitySerializer::new(
                &register,
                self.components,
                &mut self.file,
                self.indent + 1,
    ...
    fn serialize_resources(&mut self) -> Result<()> {
        writeln!(&mut self.file, "{}", Chapters::Resources)?;
        self.file.write_str("Resources are not implemented\n")?;
        Ok(())
    }
}
