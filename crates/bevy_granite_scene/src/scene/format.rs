use std::{borrow::Cow, hash::Hash, io::Read};
...
#[derive(Debug, Clone)]
pub enum BasicFormats {
    HumanVerbose,
    HumanReduced(HashMap<Cow<'static, str>, String>),
}

impl BasicFormats {
    pub fn magic(&self) -> [u8; 4] {
        match self {
            BasicFormats::HumanVerbose => *b"GHSV",
            BasicFormats::HumanReduced(_) => *b"GHSR",
        }
    }
    pub fn extract_str(&mut self, reader: &str) -> crate::Result<()> {
        match self {
            BasicFormats::HumanVerbose => {
                // verbose doesnt need to extract state from the scene file
                Ok(())
            }
            BasicFormats::HumanReduced(map) => {
                let start = Chapters::NameMap.find(reader)?;
                let end = Chapters::find_next(reader[start..].as_ref());
                let names = if let Some(end) = end {
                    &reader[start..end]
                } else {
                    &reader[start..]
                };
                for line in names.lines() {
                    ...
                    // split at the arrow or assume invalid line
                    let Some((short, long)) = line.split_once("->") else {
                        continue;
                    };
                    // trim whitespace from short name
                    let short = short.trim();
                    // trim whitespace and trailing semicolon from long name
                    let long = long.trim_matches(|c: char| c == ';' || c.is_whitespace());
                    map.insert(long.to_string().into(), short.to_string());
                }
                Ok(())
            }
        }
...
use crate::scene::{Chapters, SceneFormatError};
...
struct HumanReducedDe<'de> {
    long_name_mapping: &'de HashMap<Cow<'static, str>, String>,
}
...
impl<W: std::fmt::Write> SceneFormatWright<W> for HumanReduced {
    ...
    fn add_tail(&self, wrighter: &mut W) -> crate::Result<()> {
        if !self.used_names.is_empty() {
            writeln!(wrighter, "{}", Chapters::NameMap)?;
...
impl SceneFormatRead<&str> for HumanReduced {
    fn extract_str(&mut self, reader: &str) -> crate::Result<()> {
        let start = reader
            .find(Chapters::NameMap)
            .ok_or(SceneFormatError::MissingChapter(Chapters::NameMap))?;
